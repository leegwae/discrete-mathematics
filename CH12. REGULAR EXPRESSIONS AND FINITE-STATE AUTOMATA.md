...
$A$가 next-state function $N$과 eventual-state function $N^*$을 가진 finite-state automaton이라고 하자.$A$의 state의 집합에 대한 binary relation을 다음과 같이 정의한다: 주어진 $A$의 모든 state $s$와$t$에 대하여, 아래와 같은 경우, 그리고 오직 이 경우에만 **$s$ and $t$ are $*$-equivalent**하다고 말하며 $s\ R_*\ t$라고 쓴다: 각각의 input string $w$에 대하여,

$N^*(s,w)$와 $N^*(t,w)$는 모두 accepting state이거나 nonaccepting state이다.
...
$R_*$은 $A$의 state의 집합인 $S$에 대한 equivalence relation이다.
...
### 12.3.3 Constructing the Quotient Automaton



### 12.3.4 Equivalent Automata
